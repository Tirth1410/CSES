...
public class Planets_and_Kingdoms {
    ...
    static void dfs(int curr, int [] vis, int mark, ArrayList<Integer> [] adj) {
        if(vis[curr] != 0) return;
        vis[curr] = mark;
...
//        int test = fr.nextInt();
        ...
        for (int tests = 0; tests < test; tests++) {
            ...
            while(!stack.isEmpty()) {
                ...
                dfs(curr, vis, kingdom++, inv);
            }

            out.println((kingdom-1));
            for(int i=1; i<=n; i++) out.print(vis[i] + " ");
            out.println();
        }
    }
